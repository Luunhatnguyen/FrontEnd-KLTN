...
export default function ManageBus(props) {
  ...
  useEffect(() => {
    let loadRouter = async () => {
      ...
      try {
        let res = await API.get(endpoints["get_carrier_by_userID"](user.id));
        setBus(res.data);
        // let res = await API.get(endpoints["carriers"]);
        // setBus(res.data);
  ...
  results = (
    <>
      <h3>
        Hiển thị {bus.length} trên {count} kết quả nhà xe
      </h3>
    </>
  );
    ...
    >
      <div className="inner-box">
        ...
        <div className="content-box">
          ...
          <h3>
            <Link
              to={`/manageBus/${props.carrier.id}`}
              data-toggle="tooltip"
              title={props.carrier.id}
            >
              <p>Tên nhà xe : {props.carrier.nameCarrier}</p>
            </Link>
          </h3>
          <Row>
            <p style={{ marginTop: "-3px", marginLeft: "5px" }}>
              {props.carrier.optionID.description} - Phí :{" "}
              {formatPrice(props.carrier.optionID.fee)} VND
            </p>
          </Row>
          <Row>
            <Col xs={4} style={{ display: "inline-flex" }}>
              <FaBus />
              <p style={{ marginTop: "-3px", marginLeft: "5px" }}>
                Địa chỉ: {props.carrier.garageID.name}
              </p>
            </Col>
            <Col xs={4} style={{ display: "inline-flex" }}>
              <ImCon.ImClock />
          ...
          <div className="btn-box">
            <Link to={`/manageBus/${props.carrier.id}`}>Chi Tiết</Link>
          </div>
        </div>
      </div>
    </div>
